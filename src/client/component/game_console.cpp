...
#define material_white game::Material_RegisterHandle("white", 7)
#define con_inputDvarMatchColor new float[4] { 1, 1, 0.8, 1 }
#define con_inputCmdMatchColor new float[4] { 0.8, 0.8, 1, 1 }
...
namespace game_console
{
    namespace
    {
        ...
        void draw_input()
		{
			...
			draw_input_box(1, game::Dvar_FindVar("con_inputBoxColor")->current.vector);
			draw_input_text_and_over("IW4x64-Mod: " VERSION " >", color_title);

			con.globals.left_x = con.globals.x;
			con.globals.auto_complete_choice[0] = 0;

			game::AddBaseDrawTextCmd(con.buffer, 0x7FFFFFFF, console_font, con.globals.x,
			...
			if (matches.size() > 24)
			{
				...
				draw_hint_box(1, game::Dvar_FindVar("con_inputHintBoxColor")->current.vector);
				draw_hint_text(0, 
					utils::string::va(
			...
			else if (matches.size() == 1)
			{
				...
				const auto height = draw_hint_box(line_count, game::Dvar_FindVar("con_inputHintBoxColor")->current.vector);
				...
				if (dvar)
				{
					...
					draw_hint_text(0, game::Dvar_ValueToString(dvar, dvar->current),
						con_inputDvarMatchColor, offset);
					draw_hint_text(1, "  default", con_inputDvarMatchColor);
					...
					draw_hint_box(line_count_, game::Dvar_FindVar("con_inputHintBoxColor")->current.vector, 0, offset_y);
					draw_hint_text(0, dvars::dvar_get_domain(dvar->type, dvar->domain).data(),
						con_inputCmdMatchColor, 0, offset_y);
				}
			...
			{
				draw_hint_box(static_cast<int>(matches.size()), game::Dvar_FindVar("con_inputHintBoxColor")->current.vector);
    	...
    	void draw_output_scrollbar(const float x, float y, const float width, const float height, output_queue& output)
        {
        	...
        	draw_box(_x, y, 10.0f, height, game::Dvar_FindVar("con_outputBarColor")->current.vector);
        	...
        	draw_box(_x, y, 10.0f, _height, game::Dvar_FindVar("con_outputSliderColor")->current.vector);
        }
    	...
    	void draw_output_window()
        {
        	con.output.access([](output_queue& output)
			{
				draw_box(con.screen_min[0], con.screen_min[1] + 32.0f, con.screen_max[0] - con.screen_min[0],
					(con.screen_max[1] - con.screen_min[1]) - 32.0f, game::Dvar_FindVar("con_outputWindowColor")->current.vector);
				...
				game::R_AddCmdDrawText("IW4x64-Mod", 0x7FFFFFFF, console_font, x,
	...
	void print(const int type, const std::string& data)
	{
		...
		for (const auto& line : lines)
		{
			print_internal(type == 7 ? line : "^"s.append(std::to_string(type)).append(line));
		}
	}
